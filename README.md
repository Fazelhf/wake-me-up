...
- The bundled datasets (schema v2: per-line stations, real track edges and
  shared transfer-station keys) are generated by
  `python3 tools/build_transit_assets.py` from two community datasets —
  [mostafa-kheibary/tehran-metro-data](https://github.com/mostafa-kheibary/tehran-metro-data)
  (150 metro stations, lines 1–7, exact coordinates) and
  [MohammadAmin-Andy/iran-public-transport](https://github.com/MohammadAmin-Andy/iran-public-transport)
  (BRT lines 1 and 4). See `tools/sources/ATTRIBUTION.md`.
- "Anywhere" mode keeps the original free pin drop + Nominatim search.

## Route planner (origin → destination)

"Find your route" on the Home screen opens the journey planner:

- Pick an **origin** (a station, or *my current location* — resolved to the
  nearest station) and a **destination** station, across Metro **and** BRT.
- The planner runs Dijkstra over a (station, line) graph built from the real
  track edges: suggested line(s), the exact **transfer stations** (including
  metro↔BRT walking links at shared squares), **estimated time** and
  **distance** — plus one genuinely different alternative route when it
  exists.
- One tap arms the wake-up alarm for the chosen destination station.
- Everything is offline; the engine is pure Kotlin
  (`domain/route/RoutePlanner.kt`) with unit tests over the real bundled
  datasets (including "every station is reachable" data guards).
